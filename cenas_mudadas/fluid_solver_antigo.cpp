...
void lin_solve(int M, int N, int O, int b, float *x, float *x0, float a, float c) {
    ...
    int l = 0;
    float div = 1 / c;  // Inverso de c
    int blockSize = 4;  // Tamanho do bloco, ajustável para otimização
    int val = M + 2;
    int val2 = N + 2;
    ...
    do {
        ...
        // Processamento em blocos para elementos de paridade (j + k) % 2 == 0
                ...
                for (int ib = 1; ib <= M; ib += blockSize) {
                    for (int k = kb; k < std::min(kb + blockSize, O + 1); k++) {
                        for (int j = jb; j < std::min(jb + blockSize, N + 1); j++) {
                            for (int i = ib + (j + k) % 2; i < std::min(ib + blockSize, M + 1); i += 2) {
                                int idx = IX(i, j, k);
                                old_x = x[idx];
        ...
        // Processamento em blocos para elementos de paridade (j + k) % 2 == 1
        for (int kb = 1; kb <= O; kb += blockSize) {
            for (int jb = 1; jb <= N; jb += blockSize) {
                for (int ib = 1; ib <= M; ib += blockSize) {
                    for (int k = kb; k < std::min(kb + blockSize, O + 1); k++) {
                        for (int j = jb; j < std::min(jb + blockSize, N + 1); j++) {
                            for (int i = ib + (j + k + 1) % 2; i < std::min(ib + blockSize, M + 1); i += 2) {
                                int idx = IX(i, j, k);
                                old_x = x[idx];
                                x[idx] = (x0[idx] + a * (x[idx - 1] + x[idx + 1] +
                                                         x[idx - 86] + x[idx + 86] +
